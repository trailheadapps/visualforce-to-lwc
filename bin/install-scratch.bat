...
rem Set parameters
set ORG_ALIAS=visualforce-to-lwc

@echo:
echo Installing Visualforce to LWC scratch org (%ORG_ALIAS%)
@echo:
...
cmd.exe /c sfdx force:user:permset:assign -n Visualforce_to_LWC
call :checkForError
@echo:
...
if ["%errorlevel%"]==["0"] (
  ...
  @echo:
  cmd.exe /c sfdx force:org:open -p lightning/n/Layout
)
